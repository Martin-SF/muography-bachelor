...
for event in tqdm(range(STATISTICS), disable=False):
    # idee: data in ein 5D array packen wo momentum in [0] position in [1]
    # data arrays sind in float_type 64. vielleicht besser ungenaueren
    #   float_type zu benutzen
    ...
    current_muon = (
        f'[{event}]: position: {position} direction: (1, {phi}, {theta}) ' +
        f'momentum (MeV): {momentum} charge: {charge}\n'
    )
    ...
    try:
        if (charge == 1):
            track = prop_plus.propagate(
                init_state, max_distance, min_energy, hierarchy_condition)
        else:
        ...
        print(e)
        current_muon = (
            f'[{event}], {position}, (1, {phi}, {theta}), ' +
            f'{momentum} MeV, {charge}\n'
        )
...
plib.plot_3D_start_end(
    start_end_points[:int(1e5)], detector_pos, detector_size,
